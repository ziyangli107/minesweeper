...
class GameBoardPanel extends JPanel {
   ...
   private SquareView[][] mySquares;  // need to update display together

   private VisibleField visibleField;     // game data 
   ...
   */
   ...
   private class SquareListener extends MouseAdapter {
      ...
      private void openSquare() {
         ...
         updateAllSquaresViews();     // don't know which squares changed, so update view for all squares 
      ...
      private void changeGuessStatus() {
         ...
         if (visibleField.getStatus(mySquare.getRow(), mySquare.getCol()) != VisibleField.COVERED) {          
            ...
            int displayNum = (minesLeft >= 0) ? minesLeft : 0; // non-negative
   ...
   private class SquareView extends JLabel {
      ...
      // changes the border of the label so it looks like a button that's is or is not pushed in depending
